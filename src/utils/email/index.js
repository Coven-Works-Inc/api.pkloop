  ...
  switch (template) {
    case 'welcome':
      data = {
        ...
        html: `Welcome to PKLoop, the most efficient platform for parcel delivery. Click on the link to get started <a href="http://pkloop.herokuapp.com/api/auth/verify?token=${token}" style="text-decoration: none;">Verify Me</a>`
      }
      break
    case 'reset':
      data = {
        ...
        html: `<p>Hello ${name}, <br/> Someone requested a password reset for your account, if that was you, Click the link below to reset your password, else ignore this message. <br/> <a href="http://${headers}/api/auth/password/${token}">link</a> to set a new password.</p>`
      }
      break
    case 'updatePassword':
      data = {
